...
import type { AnimationConfig } from '../animationStore';
...
// Memoized slider that only subscribes to its specific field in the store
const AnimationSlider = memo(function AnimationSlider({
  label,
  field,
  min,
  max,
  ...
  const value = useAnimationStore((s) => s[field]);
  const updateConfig = useAnimationStore((s) => s.updateConfig);
  ...
  return (
    <Box>
      ...
      <Slider
        ...
        max={max}
        step={step}
        onChange={(_, val) => updateConfig({ [field]: val as number })}
      />
    </Box>
  );
});

// Memoized bezier display to isolate bezier string renders
const BezierStringDisplay = memo(function BezierStringDisplay() {
  const bezierX1 = useAnimationStore((s) => s.bezierX1);
  const bezierY1 = useAnimationStore((s) => s.bezierY1);
  const bezierX2 = useAnimationStore((s) => s.bezierX2);
  const bezierY2 = useAnimationStore((s) => s.bezierY2);
  const bezierString = `cubic-bezier(${bezierX1}, ${bezierY1}, ${bezierX2}, ${bezierY2})`;

  return (
    <span style={{ color: '#9c27b0', fontFamily: 'monospace', fontSize: 12 }}>
      {bezierString}
    </span>
  );
});

// Memoized right-hand interactive viewport component
...
export default function AnimationSettings() {
  const resetConfig = useAnimationStore((s) => s.resetConfig);
  ...
  return (
    <Stack spacing={2} sx={{ width: '100%', height: 'calc(100vh - 120px)', pb: 2 }}>
      ...
      <PanelGroup
        key="animation-playground-panels"
          ...
          <Box sx={{ flex: 1, overflowY: 'auto', pr: 2 }}>
            <Stack spacing={3}>
              ...
              <Paper sx={{ p: 3, borderRadius: (theme) => `${theme.shape.borderRadius}px` }}>
                ...
                <Stack spacing={2.5}>
                  <AnimationSlider label="Duration (ms)" field="duration" min={100} max={2000} step={50} valueSuffix="ms" />
                  ...
                  <Box sx={{ borderTop: '1px solid', borderColor: 'divider', pt: 2 }}>
                    ...
                    <Stack spacing={2}>
                      <AnimationSlider label="Bezier X1" field="bezierX1" min={0.0} max={1.0} step={0.05} />
                      <AnimationSlider label="Bezier Y1" field="bezierY1" min={0.0} max={2.0} step={0.05} />
                      <AnimationSlider label="Bezier X2" field="bezierX2" min={0.0} max={1.0} step={0.05} />
                      <AnimationSlider label="Bezier Y2" field="bezierY2" min={0.0} max={2.0} step={0.05} />
                    </Stack>
                  </Box>
                </Stack>
              </Paper>
              ...
              <Paper sx={{ p: 3, borderRadius: (theme) => `${theme.shape.borderRadius}px` }}>
                ...
                <Stack spacing={2.5}>
                  <AnimationSlider label="Start X Offset" field="startX" min={-150} max={150} step={5} valueSuffix="px" />
                  <AnimationSlider label="Start Y Offset" field="startY" min={-150} max={150} step={5} valueSuffix="px" />
                  <AnimationSlider label="Start Z Offset" field="startZ" min={-200} max={50} step={5} valueSuffix="px" />
                  <AnimationSlider label="Start Rotation X" field="startRotationX" min={-90} max={90} step={5} valueSuffix="°" />
                  <AnimationSlider label="Start Rotation Y" field="startRotationY" min={-90} max={90} step={5} valueSuffix="°" />
                  <AnimationSlider label="Start Rotation Z" field="startRotationZ" min={-180} max={180} step={5} valueSuffix="°" />
                  <AnimationSlider label="Start Scale" field="startScale" min={0.5} max={1.5} step={0.05} />
                </Stack>
              </Paper>
              ...
              <Paper sx={{ p: 3, borderRadius: (theme) => `${theme.shape.borderRadius}px` }}>
                ...
                <Stack spacing={2.5}>
                  <AnimationSlider label="Mid Step Timeline Percentage" field="midStepPct" min={10} max={90} step={5} valueSuffix="%" />
                  <AnimationSlider label="X Swing (Right)" field="exitXRight" min={100} max={600} step={10} valueSuffix="px" />
                  <AnimationSlider label="Y Rise/Dip" field="exitY" min={-200} max={200} step={5} valueSuffix="px" />
                  <AnimationSlider label="Z Depth" field="exitZ" min={-400} max={0} step={10} valueSuffix="px" />
                  <AnimationSlider label="Mid Rotation X" field="midRotationX" min={-360} max={360} step={10} valueSuffix="°" />
                  <AnimationSlider label="Mid Rotation Y" field="midRotationY" min={-360} max={360} step={10} valueSuffix="°" />
                  <AnimationSlider label="Mid Rotation Z" field="midRotationZ" min={-360} max={360} step={10} valueSuffix="°" />
                  <AnimationSlider label="Mid Scale" field="midScale" min={0.3} max={1.5} step={0.05} />
                </Stack>
              </Paper>
              ...
              <Paper sx={{ p: 3, borderRadius: (theme) => `${theme.shape.borderRadius}px` }}>
                ...
                <Stack spacing={2.5}>
                  <AnimationSlider label="Final X (Right)" field="finalXRight" min={10} max={400} step={5} valueSuffix="px" />
                  <AnimationSlider label="Final Y (Bottom)" field="finalY" min={-100} max={200} step={5} valueSuffix="px" />
                  <AnimationSlider label="Final Z (Depth)" field="finalZ" min={-500} max={-50} step={10} valueSuffix="px" />
                  <AnimationSlider label="Final Rotation X" field="finalRotationX" min={-360} max={360} step={10} valueSuffix="°" />
                  <AnimationSlider label="Final Rotation Y" field="finalRotationY" min={-360} max={360} step={10} valueSuffix="°" />
                  <AnimationSlider label="Final Rotation Z" field="finalRotationZ" min={-360} max={360} step={10} valueSuffix="°" />
                  <AnimationSlider label="Final Scale" field="finalScale" min={0.3} max={1.2} step={0.05} />
                </Stack>
              </Paper>
            </Stack>
          </Box>
